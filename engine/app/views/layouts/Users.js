        ...
        return (
            <View style={[styles.container, {backgroundColor: this.state.theme.primaryBackgroundColor}]} onLayout={this.onLayoutScreen}>
                ...
                <ScrollView showsVerticalScrollIndicator={true} style={{padding: 4}}>
                    <User UserData={this.state.LoadedData} horizontal={false} column={(this.state.layoutWidth > 1000) ? 5 : ((this.state.layoutWidth > 500) ? 3 : 2)} layoutWidth={this.state.layoutWidth} theme={this.state.theme}/>
                </ScrollView>
...
export default connect(({routes, scroll, language, display, player, auth}) => ({routes, scroll, language, display, player, auth}))(Users);
