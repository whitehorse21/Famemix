...
class Album extends Component {
    ...
    render() {
        ...
        if (!this.props.AlbumData.length) {
    ...
    renderCategory = ({ item, index }) => {
        if(this.props.column && ! this.props.search)
            //Gird view
            return (
                <TouchableOpacity style={[styles.album_view, {padding: 8, width: (100/this.props.column) + '%'}]} id={item.id} onPress={ () => Actions.albumShow({ album: item}) } >
                    <FastImage
                    ...
                    <View style={styles.album_info}>
                        ...
                        <Text style={[styles.textArtistName, {color: this.state.theme.textSecondaryColor}]} numberOfLines={1}>{item.artists.map(function(artist){return artist.name}).join(", ")}</Text>
                    </View>
                </TouchableOpacity>
            );
            ...
            //Search view
            return (
                <TouchableOpacity style={styles.search_album_view} id={item.id} onPress={ () => Actions.albumShow({ album: item}) } >
                    <FastImage
            ...
            return (
                <TouchableOpacity style={[styles.album_view, {marginLeft: index === 0 ? 16 : 8, marginRight: (index+1) === this.props.AlbumData.length ? 16 : 0, width: this.getItemWidth()}]} id={item.id} onPress={ () => Actions.albumShow({ album: item}) } >
                    <FastImage
...
export default connect(({routes, scroll, language, display, player, auth}) => ({routes, scroll, language, display, player, auth}))(Album);
